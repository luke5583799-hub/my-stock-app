...
st.set_page_config(page_title="HedgeFund OS | 新增 1608", layout="wide", page_icon="🏷️")
...
SECTORS = {
    ...
    "⚡ 重電與綠能": ["1513.TW", "1519.TW", "1503.TW", "1504.TW", "1609.TW", "1605.TW", "6806.TW", "9958.TW", "1608.TW"], # <-- 新增 1608.TW 到板塊清單
...
def generate_strategy(ticker, df, news_score):
    ...
    try:
        ...
        if s > -10: 
            p5_v = curr_price + s*5
            p10_v = curr_price + s*10
            p5 = f"{p5_v:.1f}"
            p10 = f"{p10_v:.1f}"
            p20 = f"{curr_price + s*20:.1f}"
            
            if p10_v > p5_v > curr_price and mfi_val > 60 and win_rate > 60:
